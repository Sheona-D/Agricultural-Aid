...
public class SlideActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void addDotsIndicator(int position){
        ...
        for (int i = 0; i< mDots.length; i++){
            ...
            mDots[i].setTextColor(getResources().getColor(R.color.colorAlmostWhite_Instructions));

            mDotsLayout.addView(mDots[i]);
        }

        if (mDots.length>0){
            mDots[position] .setTextColor(getResources().getColor(R.color.colorWhite_Instructions)); //setting currently selected dot to white
        }
    }
